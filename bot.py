...
def get_budget_progress(budgets, expense_stats):
...
def generate_ai_recommendation(
    ...
    advice = "Финансовая ситуация стабильна"

    if expense_total > personal_total:

        advice = (
            "Расходы превышают баланс. Снизь траты."
        )

    elif savings_total < personal_total * 0.2:

        advice = (
            "Накопления меньше 20% капитала."
        )

    elif expense_total > 50000:
...
def home():
    ...
    budgets_html = get_budget_progress(
        budgets,
        expense_stats
    )
...
<div class="sub-box">
<div class="sub-title">💵 Наличные</div>
<div class="sub-value">
{user["personal_cash"]} ₽
</div>
</div>
...
@app.route("/transfer_savings", methods=["POST"])
def transfer_savings():
    ...
    source_field = f"{from_account}_{wallet}"
    savings_field = f"savings_{wallet}"
    ...
    if balance >= amount:
        ...
        SET {savings_field} = {savings_field} + ?
